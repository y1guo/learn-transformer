...
class Transformer:
    def __init__(self, model: TransformerModel, tokenizer):
        self.model = model
    ...
    def predict(self, source: str, target: str):
        enc = self.tokenizer(source)
        src = torch.tensor(enc.input_ids)[None, :].to(DEVICE)  # (batch_size, seq_len)
        src_mask = torch.tensor(enc.attention_mask)[None, :].to(DEVICE) == 0  # (batch_size, seq_len)
        enc = self.tokenizer(target)
        tgt = torch.tensor(enc.input_ids)[None, :].to(DEVICE)  # (batch_size, seq_len)
    ...
    def translate(self, source: str):
        ...
        src = torch.tensor(enc.input_ids)[None, :].to(DEVICE)  # (batch_size, seq_len)
        src_mask = torch.tensor(enc.attention_mask)[None, :].to(DEVICE) == 0  # (batch_size, seq_len)
        tgt = torch.full((1, MAX_SEQ_LEN), 101).to(DEVICE)  # (batch_size, seq_len)
